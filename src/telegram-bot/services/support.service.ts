...
export class SupportService {
  ...
  async handleSupportCommand(ctx: Context, user: User): Promise<void> {
    ...
    const msg =
      'Пожалуйста, введите ваш запрос в следующем сообщении (допускается фото, видео, кружок, стикер или голосовое сообщение).\n\nЧтобы быстрее разобраться и воспроизвести проблему, пожалуйста, укажите:\n• на каком устройстве возникла проблема (телефон/планшет/ПК, модель, ОС);\n• используете ли вы VPN или прокси (если да — какой);\n• в каком приложении или браузере вы открываете бот и его версию (если знаете);\n• что именно вы делали по шагам перед возникновением проблемы;\n• примерное время, когда это произошло (дата и ваш часовой пояс)';
